...
export const chatAboutRun = async (req, res) => {
  ...
  try {
    if (!message) {
      log.warn('debugController', 'Missing message in chat request');
    ...
    if (runId) {
      log.step('debugController', '2', 'Fetching run and last 8 messages from DB');
      ...
      if (run) {
        ...
        log.warn('debugController', `Run ${runId} not found, using provided code if any`);
      }
    }
...
Be concise (1-3 sentences), specific, and reference line numbers when relevant.`;
    ...
    log.step('debugController', '4', 'Calling DeepSeek for response');
    const aiResponse = await callDeepSeek(prompt, 1024);

    if (runId && run) {
      log.step('debugController', '5', 'Saving messages to DB');
      await saveMessage(runId, userId, 'user', message);
      await saveMessage(runId, userId, 'assistant', aiResponse);
    }

    log.success('debugController', `Chat completed${runId && run ? ` for run: ${runId}` : ' (no storage)'}`);
    res.status(200).json({
      runId: runId || null,
      userMessage: message,
    ...
    log.error('debugController', 'AI chat failed', err);
